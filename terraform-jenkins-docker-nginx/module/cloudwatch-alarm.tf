...
resource "aws_sns_topic_subscription" "sns_topic_subscription" {
  ...
  protocol  = "email"
  endpoint  = "abc@gmail.com"    ### Provide your Group Email ID here.
}
...
############################# CPU-Utilization, Memory and Disk-Usage Alarm Docker Server #############################################################

resource "aws_cloudwatch_metric_alarm" "docker_server_cpu_alarm" {
  alarm_name          = "Doxker-Server-CPUUtilizationAlarm"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 2
  metric_name         = "CPUUtilization"
  namespace           = "AWS/EC2"
  ...
  depends_on = [null_resource.docker_server]

}

resource "aws_cloudwatch_metric_alarm" "docker_slave_disk_usage_alarm" {
  alarm_name          = "Docker-Server-DiskUsageAlarm"
  alarm_description   = "Alarm when disk usage on Docker Server is high"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
  metric_name         = "disk_used_percent"
  namespace           = "CWAgent"
...
resource "aws_cloudwatch_metric_alarm" "docker_server_memory_alarm" {
  alarm_name         = "Docker_Server_Memory_High_Utilization_Alarm"
  alarm_description  = "Alarm if EC2 memory usage exceeds 70%"
  metric_name        = "mem_used_percent"
  namespace          = "CWAgent"
  statistic          = "Average"
...
############################# CPU-Utilization, Memory and Disk-Usage Alarm Nginx Server #############################################################

resource "aws_cloudwatch_metric_alarm" "nginx_server_cpu_alarm" {
  alarm_name          = "Nginx-Server-CPUUtilizationAlarm"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 2
  metric_name         = "CPUUtilization"
  namespace           = "AWS/EC2"
  ...
  dimensions = {
    InstanceId = aws_instance.nginx_server.id
  }
  ...
  depends_on = [null_resource.nginx_server]

}

resource "aws_cloudwatch_metric_alarm" "nginx_server_disk_usage_alarm" {
  alarm_name          = "Nginx-Server-DiskUsageAlarm"
  alarm_description   = "Alarm when disk usage on Nginx Server is high"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
  metric_name         = "disk_used_percent"
  namespace           = "CWAgent"
...
resource "aws_cloudwatch_metric_alarm" "nginx_server_memory_alarm" {
  alarm_name         = "Nginx_Server_Memory_High_Utilization_Alarm"
  alarm_description  = "Alarm if EC2 memory usage exceeds 70%"
  metric_name        = "mem_used_percent"
  namespace          = "CWAgent"
  statistic          = "Average"
...
resource "aws_rum_app_monitor" "bankapp_rum" {
  ...
  depends_on = [aws_cloudwatch_metric_alarm.jenkins_master_memory_alarm, aws_cloudwatch_metric_alarm.jenkins_slave_memory_alarm, aws_cloudwatch_metric_alarm.docker_server_memory_alarm, aws_cloudwatch_metric_alarm.nginx_server_memory_alarm]
}
