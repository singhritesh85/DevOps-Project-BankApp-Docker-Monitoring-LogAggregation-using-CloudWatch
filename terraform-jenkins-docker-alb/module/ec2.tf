...
resource "null_resource" "jenkins_master" {
 ...
 provisioner "file" {
  ...
  provisioner "remote-exec" {
    inline = [
         "sleep 120",
         "curl -Ls https://download.newrelic.com/install/newrelic-cli/scripts/install.sh | bash && sudo NEW_RELIC_API_KEY=${var.new_relic_user_key} NEW_RELIC_ACCOUNT_ID=${var.new_relic_account_id} /usr/local/bin/newrelic install",
