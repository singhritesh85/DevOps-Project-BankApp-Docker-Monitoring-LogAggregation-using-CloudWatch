...
# Security Group for BankApp ALB
resource "aws_security_group" "bankapp_alb" {
  name        = "BankApp-ALB"
  description = "Security Group for BankApp ALB"
  vpc_id      = aws_vpc.test_vpc.id
...
#S3 Bucket to capture BankApp ALB access logs
resource "aws_s3_bucket" "s3_bucket_bankapp" {
  count = var.s3_bucket_exists == false ? 1 : 0
  bucket = "bankappalbs3bucket${var.env[0]}"
...
#BankApp S3 Bucket Server Side Encryption
resource "aws_s3_bucket_server_side_encryption_configuration" "s3bucket_encryption_bankapp" {
  count = var.s3_bucket_exists == false ? 1 : 0
  bucket = aws_s3_bucket.s3_bucket_bankapp[0].id
...
#Apply Bucket Policy to S3 Bucket For BankApp ALB access logs
resource "aws_s3_bucket_policy" "s3bucket_policy_bankapp" {
  count = var.s3_bucket_exists == false ? 1 : 0
  bucket = aws_s3_bucket.s3_bucket_bankapp[0].id
  policy = <<EOF
    {
       ...
       "Statement": [
         ...
         "Action": "s3:PutObject",
         "Resource": "arn:aws:s3:::bankappalbs3bucketdev/application_loadbalancer_log_folder/AWSLogs/${data.aws_caller_identity.G_Duty.account_id}/*"
         }
       ]
    }
  EOF

  depends_on = [aws_s3_bucket_server_side_encryption_configuration.s3bucket_encryption_bankapp]
}

#Application Loadbalancer BankApp
resource "aws_lb" "test-application-loadbalancer-bankapp" {
  name               = "bankapp"
  internal           = var.internal
  load_balancer_type = var.load_balancer_type
  security_groups    = [aws_security_group.bankapp_alb.id]           ###var.security_groups
  subnets            = aws_subnet.public_subnet.*.id
  ...
  depends_on = [aws_s3_bucket_policy.s3bucket_policy_bankapp]
}

#Target Group of Application Loadbalancer BankApp
resource "aws_lb_target_group" "target_group_bankapp" {
  name     = "bankapp"
  port     = var.instance_port      ##### Don't use protocol when target type is lambda
  protocol = var.instance_protocol  ##### Don't use protocol when target type is lambda
  vpc_id   = aws_vpc.test_vpc.id
...
##BankApp Application Loadbalancer listener for HTTP
resource "aws_lb_listener" "alb_listener_front_end_HTTP_bankapp" {
  load_balancer_arn = aws_lb.test-application-loadbalancer-bankapp.arn
  port              = "80"
  protocol          = "HTTP"

  default_action {
    type             = var.type[1]
    target_group_arn = aws_lb_target_group.target_group_bankapp.arn
     redirect {    ### Redirect HTTP to HTTPS
      port        = "443"
      protocol    = "HTTPS"
      status_code = "HTTP_301"
...
##BankApp Application Loadbalancer listener for HTTPS
resource "aws_lb_listener" "alb_listener_front_end_HTTPS_bankapp" {
  load_balancer_arn = aws_lb.test-application-loadbalancer-bankapp.arn
  port              = "443"
  protocol          = "HTTPS"
  ssl_policy        = var.ssl_policy
...
## EC2 Instance1 attachment to BankApp Target Group
resource "aws_lb_target_group_attachment" "ec2_instance1_attachment_to_tg_bankapp" {
  target_group_arn = aws_lb_target_group.target_group_bankapp.arn
  target_id        = aws_instance.docker_server.private_ip               #var.ec2_instance_id[0]
  port             = var.instance_port
}
